...
if pop_file and facility_file:
    ...
    SIMILARITY_THRESHOLD = 70  # 유사도 기준

    def match_region_safe(region, candidates, threshold=SIMILARITY_THRESHOLD):
        match, score = process.extractOne(region, candidates)
        return match if score >= threshold else ""  # 기준 미달 시 빈값 처리

    fac_regions = fac_df[fac_x].tolist()
    pop_df["매칭지역"] = pop_df[pop_x].apply(lambda x: match_region_safe(x, fac_regions))

    # 빈값 제외 후 병합
    merged = pd.merge(
        pop_df[[pop_y, "매칭지역"]].query('매칭지역 != ""'),
        fac_df[[fac_y, fac_x]],
        left_on="매칭지역",
        right_on=fac_x,
        how="inner"
